      ...
      <Container>
        {contents &&
          contents
            ...
            .filter(
              (content) => !filter.fulltime || content.contract === 'Full Time',
            )
            .map((content, _index) => <Card key={_index} {...content} />)}
      </Container>
    </>
  );
};
...
const Card = (props) => {
  ...
  return (
    <CardContainer dark={dark} color={props.logoBackground}>
      <img src={props.logo} alt="" />
      <h2 className="subtitle">
        {props.postedAt} . {props.contract}
      </h2>
      <h1 className="title">
        <Link to={`/offer/${props.id}`}>{props.position}</Link>
      </h1>
